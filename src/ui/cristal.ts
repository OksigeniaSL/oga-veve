...
function horizonte(
  ...
  for (let g = -30; g <= 30; g += 5) {
    ...
    if (g % 10 === 0) {
      escala += `<text x="${cx - largo - 6}" y="${-g * porGrado + 4}" class="cr__paso-cifra" text-anchor="end">${Math.abs(g)}</text>`;
      escala += `<text x="${cx + largo + 6}" y="${-g * porGrado + 4}" class="cr__paso-cifra">${Math.abs(g)}</text>`;
    }
  }
  let arco = "";
  ...
  const tira = marcas
    .map((m) => {
      ...
      return (
        ...
        (m.rotula
          ? `<text x="${w - 16}" y="${yy + 5}" class="cr__cifra" text-anchor="end">${m.valor}</text>`
          : "")
      );
    })
    .join("");
    ...
    <g transform="translate(${x} ${y})">
      ...
      <g clip-path="url(#${yo}-ias)">
        <g data-tira="ias" data-medio="${h / 2}" data-porunidad="${POR_NUDO}"
          ...
          ${tira}
          <g data-bug="v1">${bug(w)}</g>
          <g data-bug="vr">${bug(w)}</g>
          <g data-bug="vref">${bug(w)}</g>
        </g>
...
function lectura(w: number, h: number, que: string): string {
   ...
   */
  return `
    <path data-alerta="${que}" class="cr__caja" d="M0 ${h / 2 - 18} l${w} 0 l0 36 l${-w} 0 Z" />
    <text data-cristal="${que}" x="${w - 8}" y="${h / 2 + 9}" class="cr__valor" text-anchor="end"></text>
  `;
}
...
function cintaDeAltitud(
  ...
  const tira = marcas
    .map((m) => {
      ...
      return (
        ...
        (m.rotula
          ? `<text x="${largo + 4}" y="${yy + 5}" class="cr__cifra">${m.valor}</text>`
          : "")
      );
    })
    .join("");
  const tambor = [-1, 0, 1]
    .map(
      (k) =>
        `<text data-tambor-cifra="${k + 1}" x="${w - 4}" y="${h / 2 + 9 + k * 26}" class="cr__valor" text-anchor="end"></text>`,
    )
    .join("");
  return `
    <g transform="translate(${x} ${y})">
      ...
      <path class="cr__caja" d="M0 ${h / 2 - 18} l${w} 0 l0 36 l${-w} 0 Z" />
      <text data-cristal="alt" x="${w - 36}" y="${h / 2 + 9}" class="cr__valor" text-anchor="end"></text>
...
function variometro(
  ...
  return `
    <g transform="translate(${x} ${y})">
      <rect width="${w}" height="${h}" rx="3" class="cr__ventana" />
      ${marcas}
      <text x="${w / 2}" y="11" class="cr__rotulo cr__rotulo--menudo" text-anchor="middle">VS</text>
      <text x="${w / 2}" y="${h - 4}" class="cr__rotulo cr__rotulo--menudo" text-anchor="middle">${Math.round(c.vsiMax / 1000)}</text>
...
function cintaDeRumbo(
  ...
  for (let g = 0; g <= 720; g += 5) {
    ...
    if (g % 30 === 0) {
      tira += `<text x="${xx}" y="${h - 10}" class="cr__cifra" text-anchor="middle">${((g % 360) / 10).toFixed(0).padStart(2, "0")}</text>`;
    }
  }
  return `
    <g transform="translate(${x} ${y})">
      ...
      <path class="cr__caja" d="M${w / 2 - 26} 0 l52 0 l0 20 l-52 0 Z" />
      <text data-cristal="hdg" x="${w / 2}" y="${15}" class="cr__valor" text-anchor="middle"></text>
          ...
          : g === 90
            ...
            : g === 180
              ...
              : g === 270
                ? "W"
                : String(g / 10);
      carta += `<text x="${Math.sin(a) * (r - 22)}" y="${-Math.cos(a) * (r - 22) + 5}" class="cr__cifra" text-anchor="middle">${letra}</text>`;
    }
  }
  return `
...
export function pantallaDeNavegacion(ancho: number, alto: number): string {
  ...
  return `
    ...
    <text data-cristal="nd-rumbo" x="${cx}" y="22" class="cr__valor" text-anchor="middle"></text>
    <text x="${cx}" y="36" class="cr__rotulo" text-anchor="middle">HDG</text>
    <text data-cristal="viento" x="12" y="22" class="cr__aux"></text>
    <text data-cristal="distancia" x="${ancho - 12}" y="22" class="cr__aux" text-anchor="end"></text>
  `;
}
...
export function pantallaDeMotores(
  ...
  return `
    ...
    <text x="${ancho / 2}" y="20" class="cr__rotulo" text-anchor="middle">${c.rotulo}</text>
    ${diales}
    ...
    <text data-cristal="reversa" x="${ancho - 14}" y="${yTren + 13}"
          class="cr__reversa" text-anchor="end" visibility="hidden">REV</text>
  `;
}
...
function mandoDeMotor(x: number, y: number, ancho: number, n: number): string {
  ...
  return `
    <g transform="translate(${x} ${y})">
      <text x="${ancho / 2}" y="-8" class="cr__rotulo cr__rotulo--menudo"
...
function dialDeMotor(cx: number, cy: number, r: number, i: number): string {
  ...
  return `
    <g data-motor="${i}">
      ...
      <text data-motor-cifra="${i}" x="${cx}" y="${cy + r + 18}" class="cr__valor" text-anchor="middle"></text>
      <text x="${cx}" y="${cy + 4}" class="cr__rotulo" text-anchor="middle">${i + 1}</text>
    </g>
  `;
}
  ...
  for (let k = 0; k <= 3; k++) {
    ...
    detentes += tumbada
      ? `<line x1="${d}" y1="0" x2="${d}" y2="${h}" class="cr__marca" />` +
        `<text x="${d}" y="${h + 15}" class="cr__rotulo" text-anchor="middle">${k * 10}</text>`
      : `<line x1="0" y1="${d}" x2="${w}" y2="${d}" class="cr__marca" />` +
        `<text x="${w + 5}" y="${d + 4}" class="cr__rotulo">${k * 10}</text>`;
  }
  return `
    <g transform="translate(${x} ${y})">
      <text x="${tumbada ? -10 : w / 2}" y="${tumbada ? h - 8 : -8}"
            class="cr__rotulo" text-anchor="${tumbada ? "end" : "middle"}">FLAP</text>
...
export function columnaDeMotor(ancho: number, alto: number, c: Cuadro): string {
  ...
  return `
    ...
    <text x="${ancho / 2}" y="22" class="cr__rotulo" text-anchor="middle">${c.rotulo}</text>
    ${diales}
    ...
    barras += `
      <g data-motor="${i}">
        ...
        <text data-motor-cifra="${i}" x="${bx + anchoBarra / 2}" y="${24 + altoBarra + 18}"
              class="cr__valor" text-anchor="middle"></text>
      </g>`;
  }
  return `
    <rect data-fondo="motor" width="${ancho}" height="${alto}" rx="4" class="cr__franja" />
    <text x="${ancho / 2}" y="16" class="cr__rotulo" text-anchor="middle">${c.rotulo}</text>
    ${barras}
    ...
    <text data-cristal="reversa" x="${ancho / 2}" y="${alto - 8}"
          class="cr__reversa" text-anchor="middle" visibility="hidden">REV</text>
  `;
}
