...
describe("curva de sustentación", () => {
  const aero = { cl0: PYKASU.aero.cl0, clAlpha: PYKASU.aero.clAlpha };
  const stall = PYKASU.aero.alphaStall;
  ...
  it("es simétrica en signo", () => {
    expect(liftCoefficient(-0.1, aero, stall)).toBeLessThan(
      liftCoefficient(0, aero, stall),
    );
  });
});
...
describe("carrera de despegue", () => {
  it("acelera con motor a tope y acaba despegando", () => {
    ...
    expect(model.state.position.y).toBeGreaterThan(PYKASU.gearHeight + 8);
  });
  ...
  it("no despega tirando de la palanca desde parado", () => {
    ...
    expect(model.state.onGround).toBe(true);
    expect(model.state.alpha).toBeLessThanOrEqual(PYKASU.maxGroundPitch + 1e-6);
  });
});
...
describe("ayudas de pilotaje", () => {
  it("el modo arcade retrasa la entrada en pérdida", () => {
    ...
    expect(Math.abs(arcade.state.alpha)).toBeLessThan(
      Math.abs(pilot.state.alpha),
    );
  });
});
