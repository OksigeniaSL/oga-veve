...
function drop(height: number, assist: number, bankDegrees = 0) {
  ...
  model.reset({
    position: new Vector3(0, PYKASU.gearHeight + height, 0),
    heading: 0,
    airspeed: 34,
  });
  if (bankDegrees) {
...
describe("registro de la toma", () => {
  ...
  it("registra la velocidad de descenso de la toma", () => {
    ...
    expect(drop(12, 0).touchdownSinkRate).toBeGreaterThan(
      drop(0.4, 0).touchdownSinkRate,
    );
  });
});
