...
describe("los peldaños intermedios se distinguen", () => {
  ...
  function bankAfterRelease(tier: (typeof TIERS)[number]): number {
    ...
    return Math.abs(
      (Math.asin(Math.max(-1, Math.min(1, -right.y))) * 180) / Math.PI,
    );
  }
  ...
  it("Taguato aguanta menos que Tukã en la toma, y Taguato Ruvicha menos aún", () => {
    expect(TUKA.assists.crashTolerance).toBeGreaterThan(
      TAGUATO.assists.crashTolerance,
    );
    expect(TAGUATO.assists.crashTolerance).toBeGreaterThan(
      TAGUATO_RUVICHA.assists.crashTolerance,
    );
  });
